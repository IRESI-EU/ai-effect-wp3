...
class OrchestrationService:
    ...
    def execute_workflow(self, blueprint_path: str, initial_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _execute_graph(self, graph: ExecutionGraph, initial_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_node(self, graph_node: GraphNode, initial_data: Dict[str, Any]):
        ...
        try:
            # Compile proto files
            pb2_module, pb2_grpc_module = self.grpc_service.compile_proto(node.proto_uri, self.use_case_dir)

            # Create gRPC stub
            stub, channel = self.grpc_service.create_grpc_stub(node, operation, pb2_grpc_module)

            # Prepare request data
            request_data = self._prepare_request_data(graph_node, initial_data)

            # Execute gRPC call
            response = self.grpc_service.call_grpc_method(stub, operation, pb2_module, request_data)
    ...
    def _prepare_request_data(self, graph_node: GraphNode, initial_data: Dict[str, Any]) -> Dict[str, Any]:
        """Prepare request data for node execution.

        - Start nodes use initial_data
        - Dependent nodes collect outputs from all upstream dependencies
        """
        # Start nodes: use provided initial_data
        if not graph_node.dependencies:
            self.logger.info(f"Node {graph_node.key} is a start node (no dependencies)")
            return initial_data if initial_data else {}

        # Dependent nodes: collect outputs from all upstream dependencies
        request_data = {}

        for dep_node in graph_node.dependencies:
            dep_key = dep_node.key
            if dep_key in self._execution_results:
                dep_output = self._execution_results[dep_key]

                # Merge all fields from dependency output into request
                # Field names in protobuf output/input messages should match
                for field_name, field_value in dep_output.items():
                    request_data[field_name] = field_value
            else:
                self.logger.warning(f"Dependency {dep_key} not found in execution results!")
