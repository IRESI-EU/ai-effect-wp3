...
class GrpcService:
    ...
    def compile_proto(
        self, proto_path: str, use_case_dir: str
    ) -> tuple[Any, Any]:
        ...
        if proto_path in self._compiled_modules:
            logger.debug(f"Using cached compiled proto: {proto_path}")
        ...
        logger.info(f"Compiling proto file: {full_proto_path}")
        ...
        try:
            # Compile all proto files in the directory first (to handle dependencies)
            proto_dir = full_proto_path.parent
            for proto_file in proto_dir.glob("*.proto"):
                self._compile_proto_file(str(proto_file), temp_dir)

            # Load ALL pb2 modules into sys.modules first (to handle imports)
            for pb2_file in Path(temp_dir).glob("*_pb2.py"):
                module_name = pb2_file.stem  # e.g., "common_pb2"
                spec = importlib.util.spec_from_file_location(
                    module_name, str(pb2_file)
                )
                module = importlib.util.module_from_spec(spec)
                sys.modules[module_name] = module
                spec.loader.exec_module(module)
                logger.debug(f"Loaded {module_name} into sys.modules")

            # Now load compiled modules for the requested proto
            ...
            logger.error(f"Failed to compile proto {proto_path}: {e}")
            raise RuntimeError(f"Proto compilation failed: {e}") from e
    ...
    def _compile_proto_file(self, proto_path: str, output_dir: str):
        ...
        try:
            ...
            logger.debug(f"Running protoc with args: {' '.join(protoc_args[1:])}")

            result = protoc.main(protoc_args)
            if result != 0:
                raise RuntimeError(f"protoc failed with exit code: {result}")

            logger.debug(f"Proto compilation successful: {proto_file.name}")

        except ImportError as e:
            raise RuntimeError(
                "grpc_tools not available - install with: pip install grpcio-tools"
            ) from e
        except Exception as e:
            raise RuntimeError(f"Proto compilation failed: {e}") from e

    def _load_compiled_modules(
        self, proto_filename: str, output_dir: str
    ) -> tuple[Any, Any]:
        ...
        if not pb2_grpc_path.exists():
            raise FileNotFoundError(
                f"Compiled pb2_grpc file not found: {pb2_grpc_path}"
            )
        ...
        logger.debug(f"Loaded compiled modules for {proto_name}")
    ...
    def create_grpc_stub(
        ...
        # Find service that contains the operation
        service_name = self._infer_service_name(
            operation.operation_name, pb2_grpc_module
        )

        logger.debug(
            f"Inferred service name: {service_name} for operation: "
        ...
        logger.debug(f"Creating gRPC stub for {service_name} at {address}")
    ...
    def _infer_service_name(
        self, operation_name: str, pb2_grpc_module: Any
    ) -> str:
        ...
        available_stubs = [
            attr for attr in dir(pb2_grpc_module) if attr.endswith("Stub")
        ]
        raise RuntimeError(
            f"No service stub found containing operation '{operation_name}'. "
            f"Available stubs: {available_stubs}"
        )
    ...
    def call_grpc_method(
        ...
        if not hasattr(stub, operation.operation_name):
            raise AttributeError(
                f"Operation {operation.operation_name} not found in stub"
            )
        ...
        for field_name, field_value in request_data.items():
            if hasattr(request, field_name):
                ...
                # Repeated fields: check if value has length and isn't string/bytes
                ...
                logger.warning(
                    f"Field {field_name} not found in {operation.input_message_name}"
                )

        logger.info(f"Calling gRPC method: {operation.operation_name}")
        ...
        try:
            ...
            logger.info(f"gRPC call successful: {operation.operation_name}")
            return response
